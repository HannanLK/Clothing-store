...
<body class="bg-gray-100">
            ...
            <?php if (!empty($data['products'])): ?>
                <?php foreach ($data['products'] as $product): ?>
                    <div class="product-item bg-white rounded-lg shadow-md p-2" id="product-details-<?= $product['id'] ?>">
                        ...
                        <img src="/clothing-store/public/images/accessories/<?= $product['image'] ?>" alt="<?= htmlspecialchars($product['name']) ?>" class="w-full h-64 object-cover rounded-lg mb-2"><br>
    ...
    <script>
        ...
        document.getElementById('sortProducts').addEventListener('change', function() {
            ...
            window.location.href = `/clothing-store/public/admin/accessories?sort=${sortOption}`;
        });
