<?php $title = "Womens Collection"; ?>
<div>
    <!-- Banner Image -->
    <div class="relative w-full h-80">
        <img src="<?= BASE_URL ?>images/banners/bannerWomens.png" alt="women's banner img" class="w-full h-full object-cover">
        ...
        <div class="absolute bottom-5 right-5 flex items-center bg-opacity-70 px-4 py-2 rounded-lg">
            <label for="sortOptions" class="font-semibold mr-3 text-black">SORT BY:</label>
    ...
    <div id="main-content" class="container mx-auto p-5">
            ...
            <?php if (!empty($products)): ?>
                <?php foreach ($products as $product): ?>
                    <div class="relative product-card rounded-lg p-5">
                        <div class="relative">
                            <img src="<?= BASE_URL ?>images/womens/<?= $product['image'] ?>" alt="<?= htmlspecialchars($product['name']) ?>" class="w-full h-full object-cover mb-3 rounded-md shadow-md">
        ...
        <div id="productModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50">
            <div class="modal-content bg-white mx-auto my-10 p-5 border border-gray-300 rounded-lg max-w-2xl flex">
                ...
                <div class="modal-details w-1/2 pl-5 text-justify">
        ...
        <script>
            ...
            function fetchProductDetails(productId) {
                fetch(`<?= BASE_URL ?>product/details?id=${productId}`)
                    ...
                    .then(product => {
                        ...
                        document.getElementById('modalProductImage').src = `<?= BASE_URL ?>images/womens/${product.image}`;
            ...
            document.getElementById('sortOptions').addEventListener('change', function() {
                ...
                fetch(`<?= BASE_URL ?>womens?sort=${sortOption}`, {
                    method: 'GET',
                    headers: {
                ...
                .then(products => {
                    ...
                    products.forEach(product => {
                        productContainer.innerHTML += `
                            <div class="relative product-card rounded-lg p-5">
                                <div class="relative">
                                    <img src="<?= BASE_URL ?>images/womens/${product.image}" alt="${product.name}" class="w-full h-96 object-cover mb-3 rounded-md shadow-md">
